...
def handle_merge_conflict():
    """Guides user through resolving Git merge conflict"""
    type_print("\nLet's fix your merge conflict.")

    # Offer to explain merge conflict
    ...
    # Check if user has unmerged files
    ...
    # Show example of conflict markers
    ...
    # Confirm if user has finished editing
    ...
    # Instruct user on next steps - stage and commit
    type_print("\nNow run:")
...
def handle_push_pull_errors():
    ...
    while True:
        ...
        # Error Handling - loop back if invalid input entered
        else:
...
def handle_undo_commit():
    """Guides user through undoing different types of commits"""
    type_print("\nLet's undo a commit.")
...
def handle_other():
    """Handles requests outside chatbot's scope"""
